...
public class EditReviewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        review = ((SharedState) getApplicationContext()).editingReview;
        rate = getIntent().getIntExtra(RouteActivity.RATE, 0);
        if (rate == 0) rate = review.getRate();
    ...
    private void uiInit() {
        ...
        // Show delete btn
        MaterialButton deleteBtn = findViewById(R.id.delete_review);
    ...
    private void uiSetInput() {
        Objects.requireNonNull(msgInput.getEditText()).setText(review.getMsg());
    }
    ...
    private void uiSetImages() {
        ...
        for (int i = 0; i < review.getImages().size(); i++) {
            Bitmap bitmap = review.getImages().get(i);
            images.add(bitmap);
            updateImageView(bitmap, gallery, images.size() - 1 + i, findViewById(R.id.leave_review_select_items_toolbar).findViewById(R.id.topAppBar));
        }
        if (images.size() > 0) gallery.setVisibility(View.VISIBLE);
    ...
    private void uiSetClickListeners() {
        ...
        doneBtn.setOnClickListener(v -> saveReview());
        ...
        deleteBtn.setOnClickListener(v -> {
            new MaterialAlertDialogBuilder(this)
                ...
                .setMessage(R.string.delete_review_warning)
                .setNeutralButton(R.string.cancel, null)
                .setPositiveButton(R.string.delete, (dialog, which) -> deleteReview())
                .show();
        });
    ...
    private void deleteReview() {
        route.removeReview(review.getId(), deleted -> {
            if (deleted) finish();
            else Toast.makeText(this, R.string.could_not_delete_review, Toast.LENGTH_SHORT).show();
        });
    }
}